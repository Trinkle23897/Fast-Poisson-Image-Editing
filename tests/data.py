...
"""Utilities for generating and downloading test data."""
...
def download(links: list[tuple[str, str]]) -> None:
    """Download fixture files that are not already present."""
...
def square(x: int) -> None:
    """Create a white square image for a benchmark size."""
    r = int((4**x) ** 0.5 + 2)
...
def circle(x: int) -> None:
    """Create a filled circle image for a benchmark size."""
    r = int(((4**x) * 4 / np.pi) ** 0.5 + 2)
