...
"""Generate benchmark and report plots for the documentation."""
...
def reset_data(new_col=None):
    """Reset the in-memory plotting table."""
    global data
    data = {
        "Problem Size": [],
        "Time per Op (ns)": [],
...
def parse_table(
    raw: str, ttype: str, new_col: str | None = None
) -> tuple[np.ndarray, int, int]:
    """Parse a benchmark markdown table into plot-ready arrays."""
    ...
    for k, v in result.items():
        v = v / base / 5000 * 1e9
        t += v.tolist()
        for n, i, h in zip(base, v, head, strict=False):
...
def benchmark():
    """Render the main benchmark comparison figure."""
    global data
    reset_data()
    sep = "<!--benchmark-->"
    ...
    axes = g.axes.flatten()
    for ax, b in zip(axes, base, strict=False):
        # b, tmin, tmax = b
        b = b[0]
        y = np.logspace(math.log10(tmin), math.log10(tmax), 6)
        y = (y * 100).astype(int) / 100.0
...
def ablation(backend, new_col):
    """Render an ablation plot for a backend-specific tuning axis."""
    global data
    reset_data(new_col)
    sep = f"<!--{backend}-->"
    raw = open("../../benchmark.md").read().split(sep)
...
def parse_table2(raw: str):
    """Parse the report markdown table into long-form plot data."""
    data = {
        "Backend": [],
        "Method": [],
        "Time per Op (ns)": [],
    ...
    for tpo, cmpo in [(raw[1], raw[2]), (raw[4], raw[5])]:
        ...
        for b, t, c in zip(base, tpo, cmpo, strict=False):
...
def ablation2(backend):
    """Render the report plot for a backend."""
    sep = f"<!--{backend}-->"
