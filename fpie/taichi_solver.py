...
@ti.data_oriented
class EquSolver:
    """Provide a Taichi-based Jacobi equation solver."""
    ...
    def iter_kernel(self) -> int:
        """Run one Taichi Jacobi update over the equation system."""
    ...
    def error_kernel(self) -> int:
        """Accumulate the current absolute residual."""
        ti.loop_config(parallelize=self.parallelize, block_dim=self.block_dim)
        for i in range(1, self.N):
            i0, i1 = self.tA[i, 0], self.tA[i, 1]
            i2, i3 = self.tA[i, 2], self.tA[i, 3]
...
@ti.data_oriented
class GridSolver:
    """Provide a Taichi-based Jacobi grid solver."""
    ...
    def reset(
        ...
        self.fb.dense(ti.ij, (bx, by)).dense(ti.ij, (gx, gy)).dense(
            ti.k, 3
        ).place(self.ttgt)
        self.fb.dense(ti.ij, (bx, by)).dense(ti.ij, (gx, gy)).dense(
            ti.k, 3
    ...
    @ti.kernel
    def iter_kernel(self) -> int:
        """Run one Taichi Jacobi update over the grid state."""
        ti.loop_config(parallelize=self.parallelize, block_dim=self.block_dim)
