"""GUI entrypoint and interactions for Fast Poisson Image Editing."""
...
class GUI:
    ...
    def __init__(
        self, proc: BaseProcessor, src: str, tgt: str, out: str, n: int
    ):
        """Run the interactive GUI session."""
        super().__init__()
        self.xt, self.yt = 0, 0
    ...
    def source_callback(
        ...
        """Track the source-region rectangle selected by the user."""
    ...
    def target_callback(
        ...
        """Run the solver for the selected target placement."""
        if event == cv2.EVENT_LBUTTONDOWN:
            self.gui_tgt = self.tgt.copy()
            ...
            t = time.time()
            self.proc.reset(
                self.src, mask, self.tgt, (self.y0, self.x0), (y, x)
            )
