...
class BlockRBProcessor(BaseProcessor):
    """PIE block red-black GS processor."""
    ...
    def __init__(
        ...
        if backend == "openmp" and core_openmp is not None:
            core = core_openmp.BlockRBSolver(tile_size, n_cpu)

        super().__init__(gradient, rank, backend, core)

    # reset() is identical to GridProcessor.reset() -- brb uses the same
